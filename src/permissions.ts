...
export const PERMISSION_LABELS: Record<string, PermissionInfo> = {
  ...
  'plugin:list':        { id: 'plugin:list',        label: '读取已安装插件列表',     risk: 'normal' },
...
export function permissionLabel(id: string): PermissionInfo {
  return PERMISSION_LABELS[id] ?? { id, label: id, risk: 'normal' }
}
